...
if __name__ == "__main__":
    ...
    if operating_system_name == "nt":
        print("A Windows operating system has been detected. Docker does not allow direct access to USB webcam hardware, so if you want to use a locally attached camera, a RTMP streaming server will have to be created and a ffmpeg stream fed into the container. The quality of the feed from this is highly dependent on your network speed.")
        ...
        while not windows_choice_answered:
            ...
            if res == "y":
                ...
                subprocess.run(["ffmpeg", "-f", "dshow", "-i", f'video={camera_name}', "-vcodec", "libx264", "-preset", "veryfast", "-b:v", "500k", "-maxrate", "3000k", "-bufsize", "512M", "-an", "-f", "flv", "rtmp://localhost/stream/test"],
                               check=True,
        ...
        print("A Linux based machine has been detected. You can: ")
    ...
    print("Navigate to http://localhost:8080 to use Byakugan.")
